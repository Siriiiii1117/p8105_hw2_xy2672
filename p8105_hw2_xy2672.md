...
included. There are 5 unique counties included.
...
Many zipcodes are missing because there are no neighborhoods associated
with them, so `Zori_df` exclude them because there are no prices that
can be found. This is reflected in the `zipcode_df` if the
`neighborhood` variable does not have any value.
...
```
    ...
    ## 
    ## 
    ## | zip_code|county   |neighborhood                  |  jan2020|  jan2021|    change|
    ## |--------:|:--------|:-----------------------------|--------:|--------:|---------:|
